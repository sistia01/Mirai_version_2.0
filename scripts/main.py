...
from sklearn import calibration #added
...
if __name__ == '__main__':
    ...
    if args.test:
        #saving hiddens, added
        print("\nSaving test hiddens...")
        test_hiddens, test_img_paths = train.get_hiddens(test_data, model, args) if len(test_data) > 0 else {}, {}
        torch.save(test_hiddens, "/sc/arion/projects/shenl03_ml/2023_adriana_bc_risk/Mirai_test/Mirai/results/saved_hidden_20230825.pt")
            ...
            for exam, arr in zip(exams, probs):
                ...
                for i in range(args.max_followup):
                    ...
                    raw_val = arr[i]
                        ...
                        #val = callibrator[i].predict_proba([[raw_val]])[0,1] #error
                        #val = predict_proba([[raw_val]])[0,1]
                    #else:
                    val = raw_val
                    export[key] = val

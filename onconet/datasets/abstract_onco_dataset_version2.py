...
# Added for csv file reading: Adriana Sistig, July 18, 2023. 
DEVICE_TO_ID = {'GE': 1,
              'Hologic': 2}
...
class Abstract_Sinai_Dataset(data.Dataset):
    """
    ...
    def __init__(self, args, transformers, split_group):
        ...
        '''
        super(Abstract_Sinai_Dataset, self).__init__()
        ...
        try:
            ...
            if "all" not in self.args.allowed_devices:
                self.dataset = [d for d in self.dataset if (d['device_name'] if isinstance(d['device_name'], str) else d['device_name'][0])  in self.args.allowed_devices]
                print("Device information added to dataset")
        except:
    ...
    def build_path_to_device_map(self):
        ...
        df = pd.read_csv('/sc/arion/projects/shenl03_ml/2023_adriana_bc_risk/Mirai_test/Mirai/data/mssm_mammogram_metadata_for_mirai_w_device.csv')  # csv file path
        
        for index, row in df.iterrows():
            device_name = '{} {}'.format(row['device_name'], 'C-View') if 'C-View' in row['view'] else row['device_name']
            path_to_device[row['file_path']] = device_name
            exam_to_device[row['exam_id']] = device_name
